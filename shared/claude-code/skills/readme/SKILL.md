...
If no lockfile is found but `package.json` exists, ask the user. Present options in preference order: **pnpm → bun → vite (vp) → npm**. Never default to npm if another lockfile is present.
